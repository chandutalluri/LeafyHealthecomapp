/**
 * Enhanced API Hooks for analytics-reporting
...
const API_PREFIX = getApiPrefix('analytics-reporting');

interface AnalyticsReportingItem {
  id: string;
  [key: string]: any;
}

export function useAnalyticsReporting() {
  const [items, setItems] = useState<AnalyticsReportingItem[]>([]);
  ...
  const {
    ...
    queryKey: ['analytics-reporting', 'all'],
    queryFn: async () => {
      try {
  ...
  const createMutation = useMutation({
    mutationFn: async (newItem: Partial<AnalyticsReportingItem>) => {
      return await api.post(API_PREFIX, newItem);
    },
    onSuccess: (newItem) => {
      queryClient.invalidateQueries({ queryKey: ['analytics-reporting'] });
      setItems(prev => [...prev, newItem]);
      toast({
        title: 'Success',
        description: 'analytics reporting item created successfully',
      });
    },
    onError: (error) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<AnalyticsReportingItem> }) => {
    ...
    onSuccess: (updatedItem) => {
      ...
      toast({
        title: 'Success',
        description: 'analytics reporting item updated successfully',
      });
    },
    onError: (error) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, deletedId) => {
      ...
      toast({
        title: 'Success',
        description: 'analytics reporting item deleted successfully',
      });
    },
    onError: (error) => {
      if (error instanceof APIError && error.status === 401) {
