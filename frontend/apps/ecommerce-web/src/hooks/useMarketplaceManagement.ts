/**
 * Enhanced API Hooks for marketplace-management
...
const getApiPrefix = (domain) => {
  ...
  return prefixMap[domain] || '/api/generic';
};

const API_PREFIX = getApiPrefix('marketplace-management');

interface MarketplaceManagementItem {
  id: string;
  [key: string]: any;
}

export function useMarketplaceManagement() {
  ...
  const {
    ...
    queryKey: ['marketplace-management', 'all'],
    queryFn: async () => {
      try {
  ...
  const createMutation = useMutation({
    mutationFn: async (newItem: Partial<MarketplaceManagementItem>) => {
      return await api.post(API_PREFIX, newItem);
    },
    onSuccess: (newItem) => {
      queryClient.invalidateQueries({ queryKey: ['marketplace-management'] });
      setItems(prev => [...prev, newItem]);
      toast({
        title: 'Success',
        description: 'marketplace management item created successfully',
      });
    },
    onError: (error) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<MarketplaceManagementItem> }) => {
      return await api.put(`${API_PREFIX}/${id}`, data);
    },
    onSuccess: (updatedItem) => {
      ...
      toast({
        title: 'Success',
        description: 'marketplace management item updated successfully',
      });
    },
    onError: (error) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      return await api.delete(`${API_PREFIX}/${id}`);
    },
    onSuccess: (_, deletedId) => {
      ...
      toast({
        title: 'Success',
        description: 'marketplace management item deleted successfully',
      });
    },
    onError: (error) => {
      if (error instanceof APIError && error.status === 401) {
