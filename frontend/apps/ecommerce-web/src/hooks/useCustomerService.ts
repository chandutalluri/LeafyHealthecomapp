/**
 * Enhanced API Hooks for customer-service
...
const API_PREFIX = getApiPrefix('customer-service');
...
export function useCustomerService() {
  const [items, setItems] = useState<CustomerServiceItem[]>([]);
  ...
  const createMutation = useMutation({
    mutationFn: async (newItem: Partial<CustomerServiceItem>) => {
    ...
    onSuccess: (newItem) => {
      ...
      toast({
        title: 'Success',
        description: 'customer service item created successfully',
      });
    },
    onError: (error) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<CustomerServiceItem> }) => {
    ...
    onSuccess: (updatedItem) => {
      ...
      toast({
        title: 'Success',
        description: 'customer service item updated successfully',
      });
    },
    onError: (error) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, deletedId) => {
      ...
      toast({
        title: 'Success',
        description: 'customer service item deleted successfully',
      });
    },
    onError: (error) => {
      if (error instanceof APIError && error.status === 401) {
