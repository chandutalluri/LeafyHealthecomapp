/**
 * Enhanced API Hooks for notification-service
...
const API_PREFIX = getApiPrefix('notification-service');
...
export function useNotificationService() {
  const [items, setItems] = useState<NotificationServiceItem[]>([]);
  ...
  const {
    ...
    queryKey: ['notification-service', 'all'],
    queryFn: async () => {
      try {
  ...
  const createMutation = useMutation({
    mutationFn: async (newItem: Partial<NotificationServiceItem>) => {
      return await api.post(API_PREFIX, newItem);
    },
    onSuccess: (newItem) => {
      queryClient.invalidateQueries({ queryKey: ['notification-service'] });
      setItems(prev => [...prev, newItem]);
      toast({
        title: 'Success',
        description: 'notification service item created successfully',
      });
    },
    onError: (error) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<NotificationServiceItem> }) => {
    ...
    onSuccess: (updatedItem) => {
      ...
      toast({
        title: 'Success',
        description: 'notification service item updated successfully',
      });
    },
    onError: (error) => {
      if (error instanceof APIError && error.status === 401) {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, deletedId) => {
      ...
      toast({
        title: 'Success',
        description: 'notification service item deleted successfully',
      });
    },
    onError: (error) => {
      if (error instanceof APIError && error.status === 401) {
